...
- How keel-web is served outside `npm run dev`.  `open` - found at step 12. The server mounts `/api/health` and three route groups and nothing else; the client bundle is only ever served by the Vite dev server, which also proxies `/api` and is what makes the crossings same-origin. There is currently no way to run keel-web without `vite dev`, which the image-distribution constraint from c1 will eventually need. Not drift between model and code, a missing capability, and worth its own ticket.
