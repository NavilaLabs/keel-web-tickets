...
## Superseded
Ticket 7, ADR 0009. A container with its own Claude identity cannot serve a
keel-web that any developer starts on any of their repositories: the
repositories and the login both live on the host. Claude Code now runs on the
host under the developer's own login, and the named volume, the one-time
login inside the container and the devcontainer's Claude Code feature are gone
with it.
